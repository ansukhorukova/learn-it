...
See [PROJECT_MAP.md](./PROJECT_MAP.md) for the current build state (auth, boards + public boards, board view, task panel with time tracking / attachments / comments, board & task sharing, per-viewer personal task status on public boards, user profiles & competencies, people search, DM & competency chat with reply/forward, and board import from file are done; "Shared with me" and team view are still planned).
