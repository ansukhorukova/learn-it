...
**US-039…US-040 — походження.** Фідбек користувача: вона зробила свій борд `visibility=public`; інший (реальний, окремий) автентифікований користувач відкрив його і побачив статуси її тасок — те, що вона вже вивчила. Формулювання користувача: публічний борд — це фактично **навчальний шаблон**; інший користувач навчається сам, тож "те, що вивчила я, він ще не вивчив" — статус має бути в кожного свій. Час уже приватний (перевірено окремо — не тема цієї зміни).

**Затверджені рішення користувача (не переуточнювались):**
1. Персональний статус таски — ТІЛЬКИ на публічних бордах, для глядачів БЕЗ реального членства. На бордах з реальним шерингом (owner / `board_members` / `task_shares`) статус лишається СПІЛЬНИМ, як зараз. Owner публічного борду й реальні учасники між собою — теж спільний статус.
2. Стартовий стан персонального прогресу глядача — усі таски `planned` (глядач починає з нуля, ніби щойно взяв шаблон). НЕ копія стану власника.
3. Глядач публічного борду МОЖЕ додавати коментарі — знімається поточне обмеження "публічний відвідувач лише читає коментарі". Коментарі лишаються спільними й видимими всім з доступом; право додавати розширюється на будь-якого автентифікованого відвідувача публічного борду.
4. Приватність: власник борду НЕ бачить персональних статусів глядачів (жодного рядка, жодної агрегації) — та сама абсолютна приватність, що для `time_entries`. Кожен глядач бачить лише свій персональний статус.

**Рішення business-analyst (без повторного уточнення в користувача, за прецедентом US-021…024 / US-030…033 / US-034…036):**

- **Назва таблиці — `task_personal_status`** (початкова робоча назва в запиті — `task_public_progress`, перейменовано): колонка містить буквально лише `status`, а "personal/приватний" дзеркалить формулювання приватності `time_entries` ("завжди приватні"); "progress" помилково натякало б на час.
- **Overlay — лише `status`.** `position` (порядок у колонці) лишається спільним. Крос-колонкове переміщення глядачем змінює тільки його персональний `status`, ніколи `tasks.position`; переміщення в межах однієї колонки для глядача не персиститься (візуально дозволене, скидається при перезавантаженні). Альтернативи, відкинуті: JSON-колонка (не реляційно, не запитувано); "псевдо-рядок" у `board_members` (забруднює модель реального доступу, яку весь код трактує як авторизацію); поля на `time_entries` (несуміжна відповідальність, запис може не існувати); повний "форк борду" з копіюванням тасок (величезний обсяг, поза scope).
- **Резолв при читанні.** Для таски, де ефективна роль викликача РІВНО `public` (автентифікований, борд `visibility=public`, не owner, немає рядка `board_members`, немає `task_shares` на цю таску — `getTaskRole(...).role === 'public'`, `hasBoardAccess === false`), `GET /boards/:id/tasks` і `GET /tasks/:id` віддають `status` = рядок `task_personal_status` для `(task_id, user_id)`, або `'planned'`, якщо рядка нема. Для будь-якої реальної ролі — спільний `tasks.status`, без змін. Розгалуження — у сервісному шарі `tasks.service.js` (`listTasksForBoard` вже повертає `boardRole` і per-task `myRole` через `higherRole`; `getTaskForUser` вже має `role`).
- **Змішаний випадок** (глядач має `task_shares` viewer рівно на одну таску публічного борду, без членства в борді): саме ця таска показує спільний `tasks.status` (`myRole = viewer`), решта — персональний overlay (`myRole = public`). Прямий наслідок моделі US-014/US-022 AC7 ("task_shares ніколи не поширює доступ на решту борду / реальна роль завжди переважає"), прийнятно.
- **Запис — окремий ендпоінт `PUT /api/v1/tasks/:id/my-status`**, не розширення `PATCH /tasks/:id`: тримає персональний overlay і його авторизацію повністю окремо від шляху редагування спільної таски. Назва `/my-status` консистентна з наявними `/competency-chats/mine`, `.../chat/members/me`.
- **`public → private`:** рядки `task_personal_status` ЗБЕРІГАЮТЬСЯ (не чистяться). Дешево; дає відновити прогрес, якщо борд ненадовго приватизували й повернули; доступ глядач втрачає негайно в будь-якому разі через авторизаційний гейт; рядки прибираються каскадом `ON DELETE CASCADE` при видаленні таски / борду / користувача. Свідомий вибір.
- **Коментарі:** глядач `public` тепер МОЖЕ `POST` коментар; реальний `board_members` viewer лишається read-only на коментарях (наявне свідоме рішення US-016/US-019 і CLAUDE.md п.4, поза фідбеком користувача — див. відкрите питання нижче).
- **НЕ "форк борду":** без копіювання тасок, без персональних вкладень/нотаток/оцінок, без персонального порядку. Лише персональний `status` + право коментувати.

**CLAUDE.md — текстові зміни підготовлені, файл НЕ редагується цим проходом** (прецедент US-021…024 / US-025…029 / US-030…033 / US-034…036); застосування — окремий крок:
- Розділ **"Шеринг"**, абзац про публічну видимість борда: публічний відвідувач бачить статус тасок **персонально** (власний overlay, стартує з `planned`, власник його не бачить — та сама абсолютна приватність, що `time_entries`); **може додавати коментарі** (коментарі лишаються спільними й видимими всім з доступом). Реальне членство → спільний статус, як раніше.
- Розділ **"Шеринг"**, рядок "Статус таски на спільному борді спільний (один Planned/In Progress/Done стан на всіх)": додати уточнення — спільний для owner і реальних учасників; для відвідувача публічного борду без членства статус персональний.
- Розділ **"Екрани" п.3 "Board view"**: відвідувач публічного борду без членства може рухати картки між колонками (пише персональний статус), колонки відображають його персональний прогрес.
- Розділ **"Екрани" п.4 "Task panel"**, підпункт "Коментарі": замінити нюанс "viewer бачить коментарі, але не додає" — owner/collaborator і відвідувач публічного борду можуть додавати; реальний `board_members` viewer лишається read-only.
- Розділ **"Дані"**: нова таблиця `task_personal_status (id, task_id FK → tasks ON DELETE CASCADE, user_id FK → users ON DELETE CASCADE, status enum planned|in_progress|done default planned, created_at, updated_at, unique(task_id, user_id))` — персональний overlay статусу таски виключно для відвідувача публічного борду без членства; не читається й не агрегується для owner чи реальних учасників (та сама абсолютна приватність, що `time_entries`); той самий enum `task_status`, що `tasks.status`.
- Розділ **"Поведінка"**: статус таски спільний; для відвідувача публічного борду без членства — персональний, стартує з `planned`, не копіює стан власника.
- Розділ **"API"**: додати `PUT /tasks/:id/my-status` до переліку ендпоінтів (персональний статус таски для публічного глядача).

### US-039 — Персональний статус таски для глядача публічного борду + право коментувати (backend)

```
## User Story
Як автентифікований глядач публічного борду без реального членства, я хочу мати власний статус кожної таски й змінювати його, щоб проходити борд як навчальний шаблон — мій прогрес не залежить від того, що вже вивчив власник; і я хочу додавати коментарі до тасок, щоб ставити запитання за матеріалом.

## Acceptance Criteria
1. Given нова таблиця `task_personal_status` (міграція Knex): `id` uuid PK, `task_id` uuid FK → `tasks` ON DELETE CASCADE, `user_id` text FK → `users` ON DELETE CASCADE, `status` — той самий enum `task_status`, що `tasks.status`, `NOT NULL DEFAULT 'planned'`, `created_at`/`updated_at`, `UNIQUE (task_id, user_id)`, When міграція застосована, Then наявні дані не змінюються (таблиця порожня), `tasks.status` лишається джерелом спільного статусу.
2. Given я автентифікований, борд `visibility=public`, я не owner, не маю рядка в `board_members` і не маю `task_shares` на жодну таску цього борду (ефективна роль на кожній тасці = `public`, `getTaskRole(...).role === 'public'`), When `GET /api/v1/boards/:id/tasks`, Then кожна таска віддає `status` = мій рядок `task_personal_status` для цієї таски, або `'planned'`, якщо рядка немає — НЕ `tasks.status` власника.
3. Given той самий публічний глядач, When `GET /api/v1/tasks/:id`, Then `status` резолвиться так само (персональний overlay або `'planned'`).
4. Given у мене Є будь-яке реальне членство (owner / `board_members` будь-якої ролі / `task_shares` на цю таску), When ті самі GET-и, Then `status` = спільний `tasks.status`, БЕЗ overlay — реальна роль завжди переважає (той самий принцип, що US-022 AC7).
5. Given публічний глядач має `task_shares` (напр. `viewer`) рівно на одну таску борду й не має членства в борді, When `GET /api/v1/boards/:id/tasks`, Then саме ця таска показує спільний `tasks.status` (її `myRole = 'viewer'`), решта тасок — персональний overlay (їх `myRole = 'public'`); змішаний стан прийнятний і випливає з US-014.
6. Given `PUT /api/v1/tasks/:id/my-status` з тілом `{status: 'planned'|'in_progress'|'done'}` від публічного глядача (`role === 'public'`), When статус валідний, Then upsert у `task_personal_status` за `(task_id, user_id)` — ідемпотентно, race-safe (`onConflict(['task_id','user_id']).merge()`, той самий підхід, що ідемпотентний join `competency_chat_members` у US-031), 200 з `{taskId, status, updatedAt}`.
7. Given повторний `PUT .../my-status` з тим самим або іншим значенням, Then рядок оновлюється (не дублюється), `updated_at` оновлюється; персональний статус персистить між сесіями й пристроями (зберігається за `user_id` на BE).
8. Given `status` поза enum `task_status` або відсутній у тілі, When `PUT .../my-status`, Then 400 `errors.task.invalidStatus` (реюз наявного ключа).
9. Given `PUT .../my-status` викликає користувач з реальним членством (owner / collaborator / реальний viewer), When обробка, Then 403 `errors.task.personalStatusNotApplicable` (новий ключ) — персональний overlay існує лише для ролі `public`; owner/collaborator рухають спільний статус через `PATCH /tasks/:id`, реальний viewer — read-only на спільному статусі (US-016), без персонального обходу.
10. Given `PUT .../my-status` на таску, борд якої НЕ публічний і до якої я не маю доступу взагалі, Then 403 `errors.task.forbidden` (реюз); на неіснуючу таску (або невалідний uuid) — 404 `errors.task.notFound` (реюз); неавтентифікований запит — 401.
11. Given `PATCH /api/v1/tasks/:id` (спільний статус/позиція/поля таски), When викликає публічний глядач (`role === 'public'`), Then поведінка НЕ змінюється — 403 `errors.task.readOnlyAccess` як раніше (US-022 AC3); персональний статус пишеться виключно через `PUT .../my-status`.
12. Given публічний глядач змінив персональні статуси кількох тасок, When `GET /api/v1/boards/:id/tasks`, Then `columnTotals` рахуються за РЕЗОЛВНУТИМ (персональним) статусом кожної таски — власний залогований час глядача (нульовий чи ні) групується в його персональні колонки; `boardTotalSeconds` без змін (сума власного часу глядача).
13. Given `time_entries` публічного глядача, When будь-який його запит, Then поведінка US-022 AC5 незмінна — лише власні рядки/суми, нічого чужого ні в якому вигляді; персональний статус не впливає на приватність часу. Логування часу публічним глядачем — уже наявна поведінка (таймер у Task panel ніколи не гейтиться на роль, US-016), ця story її не додає й не прибирає, лише забезпечує, що `columnTotals` поважають персональний статус (AC12).
14. Given власник борду або будь-який реальний учасник, When будь-який їхній запит до будь-якого ендпоінта, Then вони НІКОЛИ не отримують `task_personal_status` — ні рядка, ні лічильника, ні агрегованої суми (та сама абсолютна приватність, що `time_entries`); жоден ендпоінт не віддає чужий персональний статус.
15. Given `POST /api/v1/tasks/:id/comments` від публічного глядача (`role === 'public'`) публічного борду, When тіло валідне (1–2000 символів після trim, US-019 AC4-5), Then коментар створюється (`author_id` = я), 201, зʼявляється у спільному хронологічному списку — гейт запису тепер пускає owner / collaborator / `public`.
16. Given `POST .../comments` від реального `board_members` viewer, When обробка, Then 403 `errors.task.readOnlyAccess` — БЕЗ ЗМІН (US-019 AC3); зміна гейту стосується виключно ролі `public`, не реального viewer.
17. Given `POST .../comments` від користувача без жодного доступу до таски, Then 403 `errors.task.forbidden` — без змін.
18. Given `GET /api/v1/tasks/:id/comments`, Then без змін — будь-яка роль (viewer+/public) бачить повний спільний список (US-019 AC1); FE-дерево відповідей (US-034) не зачіпається.
19. Given `visibility` борду перемкнено `public → private`, When це відбувається, Then рядки `task_personal_status` НЕ видаляються (лишаються на випадок повернення до `public`); наступний `GET`/`PUT .../my-status`/`POST .../comments` колишнього публічного глядача без реального членства → 403 `errors.board.forbidden` / `errors.task.forbidden` (доступ втрачено негайно, US-022 AC10).
20. Given таска / борд / користувач видаляються, Then відповідні рядки `task_personal_status` видаляються каскадно (`ON DELETE CASCADE` на `task_id` і `user_id`; видалення борду → каскад на `tasks` → каскад сюди) — жодних осиротілих рядків.
21. Given `openapi.yaml`, Then новий шлях `PUT /api/v1/tasks/:id/my-status` задокументовано разом з реалізацією: тіло `{status}`, відповідь `{taskId, status, updatedAt}`, `errors.task.personalStatusNotApplicable`; на схемах `Task` у `GET /boards/:id/tasks` і `GET /tasks/:id` зазначено, що `status` для `myRole='public'` — це персональний overlay (фолбек `planned`), не спільний `tasks.status`.

## API-поверхня
- Новий `PUT /api/v1/tasks/:id/my-status` — body `{status}`, upsert у `task_personal_status`, дозволений лише для ефективної ролі `public`. Відповідь 200 `{taskId, status, updatedAt}`. Помилки: 400 `errors.task.invalidStatus`, 403 `errors.task.personalStatusNotApplicable` (реальне членство), 403 `errors.task.forbidden` (нема доступу), 404 `errors.task.notFound`, 401.
- Змінений резолв поля `status` у `GET /api/v1/boards/:id/tasks` і `GET /api/v1/tasks/:id`: для таски з ефективною роллю `public` — значення з `task_personal_status` (фолбек `planned`); для реальних ролей — `tasks.status` як раніше. `columnTotals` у `GET /boards/:id/tasks` рахуються за резолвнутим статусом.
- Змінений гейт `POST /api/v1/tasks/:id/comments`: owner / collaborator / `public` дозволені; реальний `viewer` — 403 `errors.task.readOnlyAccess` (без змін). Технічно: `requireTaskRole(taskId, userId, 'viewer')`, далі `if (role === 'viewer') throw new ForbiddenError('errors.task.readOnlyAccess')` (заміняє поточний `requireTaskRole(..., 'collaborator')`).
- Нова таблиця `task_personal_status` — **потрібна міграція Knex** (`backend/migrations/`); реюз наявного enum `task_status`. `user_id` — тип `text` (як `task_comments.author_id`, `time_entries.user_id` = Firebase UID). Рекомендований додатковий btree-індекс на `user_id` під резолв-запит `WHERE user_id = ? AND task_id IN (...)` (unique `(task_id, user_id)` веде з `task_id`).
- Без змін: `time_entries` і його приватність, `attachments` / `can_view_attachment`, `board_members` / `task_shares`, `PATCH /tasks/:id`, `GET /boards/public`.

## Локалізація
- `errors.task.personalStatusNotApplicable` — en: "Personal progress only applies when you're viewing a public board you're not a member of.", uk: "Персональний прогрес доступний лише коли ви переглядаєте публічний борд без членства."
- Реюз (нові рядки не потрібні): `errors.task.invalidStatus`, `errors.task.forbidden`, `errors.task.notFound`, `errors.task.readOnlyAccess`, `errors.board.forbidden`.

## Відповідність scope
Виходить за межі поточного тексту CLAUDE.md — розділ "Шеринг" зараз прямо каже "Статус таски на спільному борді спільний (один Planned/In Progress/Done стан на всіх)", а абзац про публічну видимість — "коментарі (без права додавати)" для публічного відвідувача. Це свідоме, викликане фідбеком користувача звуження обох правил САМЕ для випадку "публічний борд + глядач без членства" — той самий прецедент розширення, що US-022, яка й запровадила публічну видимість. Приватність `time_entries` не послаблюється: персональний статус успадковує ту саму абсолютну приватність (AC14). Не зачіпає "Поза межами цього етапу": доступ лишається автентифікованим (не публічні посилання для незареєстрованих), без нотифікацій, календаря чи графіків. НЕ вводить "форк борду" — жодного копіювання тасок, персональних вкладень/нотаток/оцінок чи персонального порядку; лише персональний `status` + право коментувати. CLAUDE.md текстові зміни підготовлені в розділі "походження" вище, файл не редагується цим проходом.
```

### US-040 — Інтерактивний публічний борд: персональний статус і форма коментаря для глядача (frontend)

```
## User Story
Як глядач публічного борду без членства, я хочу рухати картки між колонками й лишати коментарі прямо на борді, щоб проходити його як власний навчальний шаблон, не змінюючи оригінал власника.

## Acceptance Criteria
1. Given я відкриваю `/boards/$boardId` публічного борду, де `board.myRole === 'public'`, Then банер над бордом повідомляє, що прогрес і таймери персональні, картки можна рухати й коментувати, але сам борд не редагується (`sharing.publicViewerBanner`, оновлений текст).
2. Given публічний глядач, When рендериться картка таски (`task.myRole === 'public'`), Then контрол статусу (`<select>`) і drag-and-drop між колонками АКТИВНІ (на відміну від реального viewer, у якого вони задизейблені, US-016); кнопки "Видалити таску", "Додати таску", "Керувати доступом", перейменування/видалення борду лишаються прихованими.
3. Given публічний глядач змінює статус картки (drag між колонками або `<select>`-фолбек), When зміна застосовується, Then FE викликає `PUT /api/v1/tasks/:id/my-status {status}` (новий клієнт `setMyTaskStatus`), НЕ `PATCH /tasks/:id`; оптимістичне оновлення з відкатом і локалізованим банером помилки при невдачі (той самий патерн, що наявні `handleStatusChange` / `handleDragEnd`).
4. Given публічний глядач перетягує картку в межах тієї самої колонки (зміна порядку без зміни статусу), Then порядок оновлюється лише візуально й не персиститься (позиція на публічному борді спільна — BE не приймає персональний порядок); після перезавантаження порядок повертається до порядку власника. Крос-колонкове переміщення персиститься через персональний статус (AC3).
5. Given реальний viewer (`task.myRole === 'viewer'`, не `public`), Then поведінка картки БЕЗ ЗМІН — статус і drag задизейблені, як у US-016.
6. Given публічний глядач відкриває Task panel, Then: секція "Час" — його власний таймер і записи (без змін); секція "Коментарі" — форма додавання коментаря АКТИВНА (не банер "лише перегляд"), кнопки "Відповісти" (US-034) присутні; "Опис" і "Оцінений час" — read-only; "Вкладення" — read-only, лише `shared`/`selected` (US-022 AC4).
7. Given реальний viewer відкриває Task panel, Then секція "Коментарі" показує банер `taskPanel.comments.viewerBanner` без форми — БЕЗ ЗМІН (US-019 AC3).
8. Given `frontend/src/lib/roles.js`, Then новий хелпер `canComment(role)` = `owner | collaborator | public`; секція коментарів (форма + кнопки "Відповісти") гейтиться `canComment`, решта write-UI таски (опис, оцінка, вкладення, видалення) — наявним `canWrite` (без `public`).
9. Given публічний глядач змінив персональні статуси, Then колонки борду показують його персональний прогрес (картки в його колонках), а не власникові; лічильники часу по колонках (якщо глядач щось логував) відповідають його персональному розкладу — приходять з BE вже порахованими (US-039 AC12), FE їх не перераховує.
10. Given `board.myRole === 'public'` і глядач змінив статус, When повертається на Boards overview і знову відкриває борд, Then персональні статуси збережені (BE-персистенція, US-039 AC7).
11. Given помилка `403 errors.task.personalStatusNotApplicable` (напр. власник встиг додати глядача в `board_members` між завантаженням і дією), Then показується локалізований банер, стан картки відкочується; повторне завантаження борду покаже вже спільний статус і реальну роль.
12. Given нові/змінені UI-рядки, Then вони присутні в `locales/en.json` і `locales/uk.json` (двомовність — частина Definition of Done).

## API-поверхня
- Споживає новий `PUT /api/v1/tasks/:id/my-status` (US-039) і змінений гейт `POST /tasks/:id/comments` (US-039). Нових ендпоінтів FE не додає.

## Локалізація
- `sharing.publicViewerBanner` (ЗМІНА тексту наявного ключа з US-022) — en: "This is a public board — your task progress and timers are your own. You can move tasks and add comments, but you can't change the board itself.", uk: "Це публічний борд — ваш прогрес по картках і таймери належать лише вам. Ви можете рухати картки й додавати коментарі, але не можете змінювати сам борд."
- `boardView.publicProgress.hint` — en: "Task statuses on this board are yours alone — the owner doesn't see them.", uk: "Статуси карток на цьому борді — лише ваші, власник їх не бачить."
- Реюз: `errors.task.personalStatusNotApplicable` (US-039), `errors.generic`, наявні рядки форми коментарів (`taskPanel.comments.*`), `boardView.card.statusLabel`, лейбли колонок `boardView.column.*`.
